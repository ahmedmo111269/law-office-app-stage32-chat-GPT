...
import { ensureSearchIndex } from './search/index-builder.js';
...
export const ROUTES = Object.freeze({
  ...
  procedures: 'الإجراءات', tasks: 'المهام', announcements: 'الإعلانات والمحضرين', 'follow-ups': 'المتابعات والاتصالات', experts: 'الخبراء', settlements: 'التسويات',
...
function backupHistoryMarkup(history) {
  return history.length
    ? `<div class="activity-list">${history.map(x => `<div class="activity-row"><time>${escapeHtml(x.date || '')}</time><div><strong>${escapeHtml(x.type || '')} — ${escapeHtml(x.status || '')}</strong><p class="muted">${escapeHtml(x.fileName || '')}</p></div></div>`).join('')}</div>`
...
async function renderBackup(page) {
  ...
  <section class="card"><h2>سجل النسخ والاستعادة</h2>
    <div id="backupHistoryList">${backupHistoryMarkup(history)}</div>
  </section>`;
  ...
  const refreshHistory = async () => {
    const list = page.querySelector('#backupHistoryList');
    if (list) list.innerHTML = backupHistoryMarkup(await getBackupHistory(20));
  };
  const plainButton = page.querySelector('#backupBtn');
  plainButton.addEventListener('click', async () => {
    plainButton.disabled = true;
    try {
      await createBackup();
      info.textContent = 'تم إنشاء النسخة الاحتياطية. احتفظ بالملف في مكان آمن.';
      toast('تم إنشاء النسخة الاحتياطية وتسجيلها.');
      await refreshHistory(); // Keep the file input and password in place during exports.
    } catch (error) { toast(error.message, 'error'); }
    finally { plainButton.disabled = false; }
  });
  const encryptedButton = page.querySelector('#encryptedBackupBtn');
  encryptedButton.addEventListener('click', async () => {
    ...
    try {
      ...
      info.textContent = 'تم إنشاء النسخة المشفرة. احتفظ بكلمة المرور خارج الجهاز.';
      toast('تم إنشاء النسخة المشفرة وتسجيلها.');
      await refreshHistory();
    } catch (error) { toast(error.message, 'error'); }
    finally { encryptedButton.disabled = false; }
  });
  const invalidatePreview = () => {
    ...
    preview.textContent = '';
    info.textContent = 'تغير الملف أو كلمة المرور. عاين النسخة مجددًا قبل الاستعادة.';
  };
  page.querySelector('#restoreFile').addEventListener('change', invalidatePreview);
  page.querySelector('#backupPassword').addEventListener('input', invalidatePreview);
...
function searchRoute(store, id) {
  const map = { clients: 'clients', opponents: 'opponents', powerOfAttorneys: 'power-of-attorneys', cases: 'cases', hearings: 'hearings', judgments: 'judgments', appeals: 'appeals', caseTasks: 'tasks', procedures: 'procedures', announcements: 'announcements', executionFiles: 'execution', collections: 'financial', courtsAuthorities: 'courts', financialRecords: 'financial', feeAgreements: 'financial', experts: 'experts', settlements: 'settlements', templates: 'templates' };
...
async function renderSearch(page, initialQuery = '') {
  ...
  const draw = async ({reset=false}={}) => {
    ...
    try {
      ...
      results.innerHTML = response.rows.length ? response.rows.map(x => `<a class="search-result" href="#/${searchRoute(x.store,x.id)}"><div><strong>${escapeHtml(x.title)}</strong><div class="muted">${escapeHtml(x.label)} · معرف داخلي ${escapeHtml(x.id)}</div></div><span>↗</span></a>`).join('') : emptyState(response.degraded ? 'الفهرس غير مكتمل؛ قد توجد نتائج أقدم لم تظهر بعد. أعد البحث بعد اكتمال بناء الفهرس.' : 'لا توجد نتائج مطابقة.');
      meta.textContent = response.degraded
        ? `الفهرس غير مكتمل أو قيد البناء: البحث مؤقت ومحدود بآخر ٢٠٠٠ سجل لكل قسم، وليس حصرًا شاملاً — عرض ${response.rows.length} نتيجة.`
        : `${searchPerformanceNote()} — عرض ${response.rows.length} نتيجة.`;
...
function renderRoute(route) {
  // A slow renderer can mutate #page before its promise resolves. Merely
  // checking a generation *afterwards* caused an endless loop of competing
  // rerenders during fast hash navigation. Serialise renders and skip obsolete
  // queued routes; the latest route paints after the previous one settles.
  const token = (renderToken += 1);
  const previous = renderQueue;
  const task = (async () => {
    try { await previous; } catch { /* an older failed route must not block navigation */ }
    if (token !== renderToken) return;
    return paintRoute(route, token);
  })();
  renderQueue = task;
  return task;
}

async function paintRoute(route, token) {
  const superseded = () => token !== renderToken;
  ...
  try {
    ...
    if (superseded()) {
      // The queued latest route will paint next; never recursively enqueue it.
...
async function boot() {
  try {
    // index.html bridges the worker's message to this event. Listen before
    // asynchronous DB startup so a fast upgrade notice cannot be missed.
    window.addEventListener('lawoffice:sw-updated', () => {
      toast('تم تثبيت إصدار جديد من التطبيق. أعد تحميل الصفحة لاستخدامه.', 'success');
    });
    ...
    // Derived index: built in the background, search falls back until it is ready.
