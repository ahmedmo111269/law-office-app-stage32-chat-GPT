...
function form(existing=null, parents=[]){
  ...
  return `<form id="authorityForm" class="form-grid">
    ...
    <label class="checkbox-label"><input type="checkbox" name="active" ${isActive(existing)?'checked':''}> الجهة نشطة</label>
...
export async function renderCourtsPage(page){
  ...
  const draw=()=>{
    ...
    const list=rows.filter(r=>{const text=normalizeText(`${r.name||''} ${r.city||''} ${r.address||''} ${r.phone||''}`);return(!q||text.includes(q))&&(!tf||r.type===tf)&&(!af||(af==='active'?isActive(r):!isActive(r)))}).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'ar'));
    page.querySelector('#authorityList').innerHTML=list.length?list.map(r=>{const parent=map.get(Number(r.parentId));const children=rows.filter(x=>Number(x.parentId)===Number(r.id)).length;return `<div class="work-item"><div><strong>${esc(r.name)}</strong><span class="badge">${esc(typeLabel(r.type))}</span><span class="badge">${!isActive(r)?'غير نشطة':'نشطة'}</span><p class="muted">${esc(r.city||'—')} ${r.address?'— '+esc(r.address):''}</p><small>${parent?'الجهة الأعلى: '+esc(parent.name)+' — ':''}الفروع التابعة: ${children} ${r.phone?' — هاتف: '+esc(r.phone):''}</small></div><div class="row-actions"><button class="secondary-button" data-edit="${r.id}">تعديل</button><button class="danger-button" data-toggle="${r.id}">${!isActive(r)?'تفعيل':'تعطيل'}</button></div></div>`}).join(''):'<p class="muted">لا توجد جهات مطابقة.</p>';
    ...
    page.querySelectorAll('[data-toggle]').forEach(b=>b.addEventListener('click',async()=>{const r=rows.find(x=>Number(x.id)===Number(b.dataset.toggle));if(!r)return; if(isActive(r)){const dependents=rows.filter(x=>Number(x.parentId)===Number(r.id)&&isActive(x));if(dependents.length){toast('لا يمكن تعطيل جهة لها جهات فرعية نشطة.','error');return}}await repo(STORES.courtsAuthorities).put({...r,active:!isActive(r),updatedAt:nowISO()});toast(!isActive(r)?'تم تفعيل الجهة.':'تم تعطيل الجهة.');rows=await repo(STORES.courtsAuthorities).all({ limit: 500 });draw()}));
  };
  draw();
