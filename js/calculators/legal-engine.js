...
export const VERIFIED_RULES = Object.freeze([
  {
    ...
    source: 'قانون المرافعات المدنية والتجارية رقم 13 لسنة 1968',
...
export async function ensureSeedRules() {
  await ensureRuleSet(VERIFIED_RULES);
}

export async function listRules(category = null) {
  const rows = await repo(STORES.legalRules).all({ limit: 500 });
  return rows.filter(x => isActive(x) && (!category || x.category === category)).sort((a,b) => String(a.name).localeCompare(String(b.name), 'ar'));
}
...
export async function calculateDeadline({ ruleId, startDate, distanceDays = 0, extraSuspensionDays = 0, applyHolidays = true, applyWeekendRule = false, notes = '' }) {
  ...
  if (!isActive(rule)) throw new Error('القاعدة القانونية غير مفعلة.');
  ...
  const holidaySet = new Set(holidays.filter(x => isActive(x)).map(x => x.date));
...
export async function addHoliday(data) {
  ...
  const row = { date: data.date, name: data.name.trim(), type: data.type || 'official', active: isActive(data), sourceReference: data.sourceReference || '', sourceUrl: data.sourceUrl || '', notes: data.notes || '' };
