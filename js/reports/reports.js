...
async function iterateStore(storeName, { from = '', to = '', where = null, onRow, signal } = {}) {
  ...
  await r.scan({ limit: 10000, onRow: row => { if (!signal?.aborted && (!where || where(row))) onRow(row); } });
}
...
export async function buildReportsData(filters = {}, { signal } = {}) {
  const f = { ...normalizeReportFilters(filters), signal };
...
function renderPresetOptions(presets, selected = '') { return `<option value=""${!selected ? ' selected' : ''}>اختر إعدادًا محفوظًا</option>${presets.map((p, i) => `<option value="${i}"${p.name === selected ? ' selected' : ''}>${escapeHtml(p.name)}</option>`).join('')}`; }
...
async function exportCsv(type, filters, button) {
  ...
  try {
    ...
    a.href = url;
    a.download = `${type}-report-${todayISO()}.csv`;
    a.rel = 'noopener';
...
export function renderReportsPage(page) {
  let filters = { from: '', to: '' };
  ...
  const draw = () => {
    // Rendering a large, unfiltered report can take seconds. Do not hold the
    // route queue open; abort this bounded scan when filters or routes change.
    controller?.abort();
    controller = new AbortController();
    const { signal } = controller;
    page.innerHTML = `<section class="card"><div class="section-title"><div><h2>📑 التقارير 2.0</h2><p class="muted">التجميع يتم بقراءات محدودة عبر فهارس التاريخ، مع عدم تحميل المخازن كاملة إلى الذاكرة.</p></div><div class="section-actions"><span class="badge">DB v15</span></div></div>
      ...
      <div class="toolbar"><select id="presetSelect" class="select compact">${renderPresetOptions(readPresets(), selectedPresetName)}</select><button id="savePreset" class="secondary-button">حفظ الإعداد</button><button id="deletePreset" class="danger-button">حذف الإعداد</button></div></section>
      <section id="reportResults" aria-live="polite" aria-busy="true" class="card"><p class="muted">جاري احتساب التقرير من السجلات الفعلية…</p></section>
    ...
    const resultHost = page.querySelector('#reportResults');
    const print = page.querySelector('[data-print-report]');
    print.disabled = true; // Never print a report whose aggregates are unfinished.

    page.querySelector('#applyReport').onclick = () => {
      try {
        filters = normalizeReportFilters({ from: page.querySelector('#reportFrom').value, to: page.querySelector('#reportTo').value });
        selectedPresetName = '';
        draw();
      } catch (error) { toast(error.message, 'error'); }
    };
    page.querySelector('#clearReport').onclick = () => { filters = { from: '', to: '' }; selectedPresetName = ''; draw(); };
    page.querySelector('#savePreset').onclick = () => {
      const name = prompt('اسم الإعداد المحفوظ:');
      if (!name?.trim()) return;
      try {
        filters = normalizeReportFilters({ from: page.querySelector('#reportFrom').value, to: page.querySelector('#reportTo').value });
        const all = readPresets().filter(item => item.name !== name.trim());
        all.unshift({ name: name.trim(), ...filters });
        savePresets(all);
    ...
    page.querySelector('#deletePreset').onclick = () => {
      const value = page.querySelector('#presetSelect').value;
      if (value === '') return; // Number('') is 0: the placeholder is NOT the first preset.
      const index = Number(value);
      const all = readPresets();
      if (!Number.isInteger(index) || !all[index]) return;
      all.splice(index, 1); savePresets(all);
      selectedPresetName = '';
      toast('تم حذف الإعداد.'); draw();
    };
    page.querySelector('#presetSelect').onchange = event => {
      if (event.target.value === '') { selectedPresetName = ''; return; }
      const selected = readPresets()[Number(event.target.value)];
      if (!selected) return;
      selectedPresetName = selected.name;
      filters = { from: selected.from || '', to: selected.to || '' };
      draw();
    };
    page.querySelectorAll('[data-export]').forEach(btn => {
      btn.onclick = () => exportCsv(btn.dataset.export, filters, btn).catch(error => toast(error.message, 'error'));
    });

    buildReportsData(filters, { signal }).then(data => {
      if (signal.aborted || destroyed) return;
      resultHost.outerHTML = `<div id="reportResults" aria-live="polite"><section class="grid grid-4">${metric('القضايا النشطة', data.activeCases)}${metric('الجلسات', data.hearings)}${metric('الأحكام', data.judgments)}${metric('الطعون', data.appeals)}</section>
        ...
        <div class="grid grid-3"><section class="card"><h3>المراكز المتخصصة</h3>${metric('عمالية', data.labor)}${metric('نفقات أسرية', data.family)}${metric('إدارية', data.administrative)}${metric('جنائية', data.criminal)}</section><section class="card"><h3>الإجراءات والمتابعة</h3>${metric('إعلانات', data.announcements, `خدم منها ${data.servedAnnouncements}`)}${metric('خبراء', data.experts)}${metric('تسويات', data.settlements)}</section><section class="card"><h3>المعلومات المسجلة</h3>${metric('الأحداث', data.events)}${metric('اتفاقات الأتعاب', data.fees)}${metric('قضايا مغلقة/مؤرشفة', data.closedCases)}</section></div></div>`;
      print.disabled = false;
      bindPrintButton(page, { title: 'التقارير' });
    }).catch(error => {
      if (signal.aborted || destroyed) return;
      resultHost.textContent = `تعذر احتساب التقرير: ${error.message || error}`;
      resultHost.setAttribute('aria-busy', 'false');
      toast(error.message || 'تعذر احتساب التقرير.', 'error');
    });
  };
  draw();
