...
async function archiveRecord(storeName, rawId, label, onSaved) {
  const id = Number(rawId);
  if (!Number.isSafeInteger(id) || id <= 0) { toast('معرّف السجل غير صالح.', 'error'); return false; }
  if (!window.confirm(`أرشفة ${label}؟ يمكن استعادة السجل من صفحة الأرشيف لاحقًا.`)) return false;
  try {
    const record = await repo(storeName).get(id);
    if (!record || record.archived) { toast('السجل غير موجود أو مؤرشف بالفعل.', 'error'); return false; }
    await repo(storeName).put({ ...record, archived: 1 });
    toast(`تمت أرشفة ${label}.`);
    await onSaved?.();
    return true;
  } catch (error) {
    toast(error.message || `تعذرت أرشفة ${label}.`, 'error');
    return false;
  }
}
...
/**
 * Row renderers for the people lists.
 *
 * These two helpers were referenced by `renderClients` / `renderOpponents`
 * but never defined anywhere in the project, so both lists threw
 * `ReferenceError: clientListRow is not defined` — and, because the throw
 * happened inside `draw()` after `page.innerHTML` had already been replaced,
 * the screens stayed permanently blank as soon as the database contained a
 * single client or opponent.
 */
const initials = (name) => {
...
const metaLine = (items) =>
  `<div class="muted">${items.filter(Boolean).map((x) => escapeHtml(x)).join(' · ')}</div>`;

export function clientListRow(row) {
  const id = Number(row.id);
  return `<article class="list-row person-row">
    <div class="person-main">
      <span class="person-avatar" aria-hidden="true">${escapeHtml(initials(row.fullName))}</span>
      <div>
        <a href="#/clients?id=${id}&view=360"><strong>${escapeHtml(row.fullName || 'بدون اسم')}</strong></a>
        ${metaLine([row.nationalId ? `الرقم القومي: ${row.nationalId}` : '', row.phone1 ? `${row.phone1}` : '', row.city || ''])}
      </div>
    </div>
    <div class="row-actions">
      <a class="secondary-button" href="#/clients?id=${id}&view=360">ملف 360°</a>
      <button class="secondary-button" data-edit-client="${id}">تعديل</button>
      <button class="danger-button" data-archive-client="${id}">أرشفة</button>
    </div>
  </article>`;
}

export function opponentListRow(row) {
  const id = Number(row.id);
  return `<article class="list-row person-row">
    <div class="person-main">
      <span class="person-avatar" aria-hidden="true">${escapeHtml(initials(row.name))}</span>
      <div>
        <a href="#/opponents?id=${id}"><strong>${escapeHtml(row.name || 'بدون اسم')}</strong></a>
        ${metaLine([row.entityType || '', row.nationalId ? `الرقم القومي: ${row.nationalId}` : '', row.phone || '', row.city || ''])}
      </div>
    </div>
    <div class="row-actions">
      <button class="secondary-button" data-edit-opponent="${id}">تعديل</button>
      <button class="danger-button" data-archive-opponent="${id}">أرشفة</button>
    </div>
  </article>`;
}
...
export async function renderClients(page) {
  ...
  const [activeCount, poaCount] = await Promise.all([
    clientRepo.countByIndex('archived', 0),
    poaRepo.count()
  ]);
  ...
  const draw = async (reset = false) => {
    ...
    if (!q) {
      const result = await clientRepo.pageByIndex('archived', 0 , { direction: 'next', limit: state.pageSize, afterKey: state.afterKey, afterPrimaryKey: state.afterPrimaryKey });
      ...
      if (!indexed.size) await clientRepo.scan({ index: 'archived', query: 0, direction: 'next', limit: 1000, onRow: row => { if ([row.fullName,row.nationalId,row.phone1,row.phone2,row.email,row.address,row.city].some(v => normalizeText(v).includes(q))) indexed.set(row.id, row); } });
    ...
    const clientInfo = page.querySelector('#clientPageInfo'); if (clientInfo) clientInfo.textContent = q ? `نتائج البحث: ${currentRows.length}` : `الصفحة ${state.pageNumber}`;
    const nextClients = page.querySelector('#nextClients'); if (nextClients) nextClients.disabled = !state.hasMore || Boolean(q);
    const prevClients = page.querySelector('#prevClients'); if (prevClients) prevClients.disabled = state.pageNumber <= 1 || Boolean(q);
  };
...
export async function renderOpponents(page) {
  ...
  const activeCount = await opponentRepo.countByIndex('archived', 0);
  ...
  const draw = async reset => {
    ...
    const q = normalizeText(search.value);
    if (!q) {
      const result = await opponentRepo.pageByIndex('archived', 0 , { direction: 'next', limit: state.pageSize, afterKey: state.afterKey, afterPrimaryKey: state.afterPrimaryKey });
      currentRows = result.rows;
      state.afterKey = result.nextKey; state.afterPrimaryKey = result.nextPrimaryKey; state.hasMore = result.hasMore;
      ...
      const hits = new Map();
      await opponentRepo.scan({ index: 'archived', query: 0, direction: 'next', limit: 1000, onRow: row => {
...
async function loadClient360(clientId) {
  ...
  const clientCases = (await repo(STORES.cases).getMany(caseIds)).filter(x => x && !x.archived);
