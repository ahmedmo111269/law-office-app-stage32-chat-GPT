...
export async function verifyPin(pin) {
  // getSetting() returns the stored *value* ({hash, salt, iterations}).
  const saved = await getSetting(PIN_KEY);
  if (!saved || !saved.hash) return true;
  const value = String(pin || '');
  const r = await derivePinHash(value, saved.salt);
  unlocked = r.hash === saved.hash;
  return unlocked;
}
export function isUnlocked(){ return unlocked; }
export function lock(){ unlocked=false; showLock(); }

export async function getAutoLockMinutes(){ const s=await getSetting(AUTOLOCK_KEY); return Number(s||0); }
