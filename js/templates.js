...
function formHtml(template) {
  ...
  return `<form id="templateForm" class="form-grid">
    ...
    <label>الحالة<select name="active"><option value="true" ${isActive(x) ? 'selected' : ''}>نشط</option><option value="false" ${!isActive(x) ? 'selected' : ''}>غير نشط</option></select></label>
  ...
  function draw() {
    ...
    const filtered = templates.filter(template => {
      ...
      return (!q || haystack.includes(q))
        ...
        && (!active.value || String(isActive(template)) === active.value);
    });
    if (!filtered.length) {
    ...
    list.innerHTML = `<div class="template-list">${filtered.map(template => {
      ...
      return `<article class="template-card"><div class="template-card-main"><div class="template-card-head"><div><span class="badge">${escapeHtml(categoryLabel(template.category))}</span><strong>${escapeHtml(template.name)}</strong></div><span class="badge">${isActive(template) ? 'نشط' : 'غير نشط'}</span></div><p class="muted">${escapeHtml(template.description || 'بدون وصف')}</p><div class="template-preview">${escapeHtml(template.content).replace(/\n/g, '<br>')}</div>${vars.length ? `<div class="template-vars"><strong>المتغيرات:</strong> ${vars.map(v => `<code>{{${escapeHtml(v)}}}</code>`).join(' ')}</div>` : ''}</div><div class="row-actions"><button class="primary-button" data-copy="${template.id}">نسخ النص</button><button class="secondary-button" data-edit="${template.id}">تعديل</button><button class="danger-button" data-toggle="${template.id}">${isActive(template) ? 'تعطيل' : 'تفعيل'}</button></div></article>`;
    }).join('')}</div>`;
    ...
    list.querySelectorAll('[data-toggle]').forEach(button => button.addEventListener('click', async () => {
      ...
      if (!template) return;
      try {
        await repo(STORES.templates).put({ ...template, active: !isActive(template), updatedAt: nowISO() });
        toast(!isActive(template) ? 'تم تفعيل القالب.' : 'تم تعطيل القالب.', 'success');
        await refresh();
